...
class PostManager extends Manager
{
  ...
  public function getPosts() {
    // $bdd = getBdd();
    return $posts = $this->getBdd()->query("select id, date_format(date_creation, '%d/%m/%Y') as date_creation, title, content from post order by id DESC");
    // return $posts;
  }
  ...
  public function getLastPosts() {
    // $bdd = getBdd();
    $lastposts = $this->getBdd()->query("select id,date_format(date_creation, '%d/%m/%Y') as date_creation, title from post order by date_creation DESC limit 4");
  ...
  public function getPost(int $idPost) {
    // $bdd = getBdd();
    $post = $this->getBdd()->prepare("select post.id,date_format(post.date_creation, '%d/%m/%Y') as date_creation, post.title, post.content, post.author,adminUser.username from post join adminUser where post.author LIKE adminUser.id AND post.id=:id;");
  ...
  public function addPost(string $title, string $content, int $id) {
    // $bdd = getBdd();
    $posts = $this->getBdd()->prepare('insert into post (title, content, date_creation, author) values(:title, :content, NOW(), :author);');
  ...
  public function modifyPost(string $title, string $content, int $idPost) {
    // $bdd = getBdd();
    $posts = $this->getBdd()->prepare('update post set title= :title, content=:content where id=:id;');
  ...
  public function deletePost(int $idPost) {
    // $bdd = getBdd();
    $posts = $this->getBdd()->prepare('delete from post where id=:id;');
